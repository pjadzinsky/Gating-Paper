...
bipolar_cell_nb = 5         # number should match file bipolar_cell_#.txt
bipolar_cell_file = ""      # I need this global variable. It will be changed in data_summary
added_noise_factor = 1.0       # increases/decreases the level of the noise at the bipolar cell
...
def data_summary(letter_length_in_ms):
    ...
    noisy_g = g + added_noise_factor * noise_std * bipolar.get_noise(g.shape, save_flag=1, corr_time=None)
    ...
    # save just a few samples of g and letters (only at -.1 and .1 secs) 
    ...
    for N in letters_nb:
        print("Compute total information with {0} letters both for basal and gating".format(N))
        zeros = _np.zeros(N-1)  # needed to keep basal_discrete and gating info of the same length and aligned in time

        datapath = folders['{0}Letters'.format(N)]

        try:
            basal_discrete = _np.fromfile(os.path.join(datapath, 'mi_{0}b_0g'.format(N)))
        except:
            basal_discrete   = mi(combine_consecutive_labels(binned_g, N), combine_consecutive_labels(binned_basal, N))
            basal_discrete = _np.concatenate((zeros, basal_discrete))
            basal_discrete.tofile(os.path.join(datapath, 'mi_{0}b_0g'.format(N)))

        try:
            gating_discrete = _np.fromfile(os.path.join(datapath, 'mi_0b_{0}g'.format(N)))
        except:
            gating_discrete   = mi(combine_consecutive_labels(binned_g, N), combine_consecutive_labels(binned_gating, N))
            gating_discrete   = _np.concatenate((zeros, gating_discrete))
            gating_discrete.tofile(os.path.join(datapath, 'mi_0b_{0}g'.format(N)))
...
def plot_summary(letter_length_in_ms):
    ...
    fig = bipolar.plot_noise(g, added_noise_factor, noise_corr_time, fig_g)
    fig.savefig(os.path.join(folders['FEM'], 'Figures', 'noise_to_be_added.pdf'), transparent=True)
    ...
    conditions = zip([saccade_size]*3, [rw_step]*3, [bipolar_cell_nb]*3, [added_noise_factor]*3,
            [50]*3, [2,4,8])
    ...
    fig = plot_word_information_2(letter_length_in_ms, 8)
    fig.savefig(os.path.join(folders['8Letters'], 'Figures', 'word_information.pdf'), transparent=True)
    ...
    fig = plot_compare_gating_timing(letter_length_in_ms)
    fig.savefig(os.path.join(folders['llength'], 'Figures', 'compare_gating_timing.pdf'), transparent=True)
    ...
    #plot_total_discrete_per_time_or_rate(letter_length_in_ms, 8)

    fig = plot_corr_coef(g, letter_length_in_ms/1000)
...
def plot_g(g, num):
    ...
    fig.savefig(os.path.join(folders['FEM'], 'Figures','g.pdf'), transparent=True)
...
def plot_word_information_2(letter_length_in_ms, letters_N):
    ...
    ########################## Load all data
    datapath = folders['{0}Letters'.format(letters_N)]
...
def plot_timing_at_constant_discrete(letter_length_in_ms, letters_N):
    '''
...
def plot_total_discrete_for_words(basal_words, gating_words, suffix="", prefix=""):
    ...
    for i, (sacc, rws, bcell, added_noise, llength, letters_N) in enumerate(basal_words):
        try:
            tax = _np.arange(sim_start_t, sim_end_t, llength/1000)#_get_ploting_TAX(llength/1000)
            
            datapath = get_folder(sacc, rws, bcell, added_noise, llength, letters_N)
    ...
    mode = ['b','b--', 'b:', 'b-.']
    for (sacc, rws, bcell, added_noise, llength, letters_N) in gating_words:
        try:
            tax = _np.arange(sim_start_t, sim_end_t, llength/1000)#_get_ploting_TAX(llength/1000)
...
def plot_discrete_and_cond_discrete(letter_length_in_ms, lettersN):
    '''
...
def plot_compare_gating_timing(letter_length_in_ms):
    ...
    letter_length = letter_length_in_ms/1000
    datapath = folders['8Letters']
    tax = _np.arange(sim_start_t, sim_end_t, letter_length)
    mi_8b_0g = _np.fromfile(os.path.join(datapath, 'mi_8b_0g'))
...
def plot_integrated_information(letter_length_in_ms, letters_N_list):
    ...
    datapath = path_template.format(letter_length_in_ms)
...
def plot_bits_per_spike(words):
    ...
    for (letter_length_in_ms, letters_N) in words:
        ...
        # path to data
        datapath = 'Inputs/{0}ms letter_length/'.format(letter_length_in_ms)
...
def plot_discrete_to_firing_rate_ratio(letter_length_in_ms):
    '''
...
def plot_total_discrete_per_time_or_rate(letter_length_in_ms, letters_N):
    '''
...
def get_discrete_per_spike(letter_length_in_ms, letters_N):
    '''
    ...
    int_discrete_basal, int_discrete_gating = _integrate_information(letter_length_in_ms, letters_N)
...
def make_datafolders(letter_length_in_ms):
    '''
    ...
    '''
    FEM_folder = "Sacc={0}, RWS={1}".format(saccade_size, rw_step)
    noise_folder = "Bcell={0}, added_noise={1}".format(bipolar_cell_nb, added_noise_factor)
    llength_folder = "{0}ms Length".format(letter_length_in_ms)

    folders = {
            'Results': os.path.join(root_path, 'Results'), 
            'FEM': os.path.join(root_path, 'Results', FEM_folder), 
            'noise': os.path.join(root_path, 'Results', FEM_folder, noise_folder),
            'llength': os.path.join(root_path, 'Results', FEM_folder, noise_folder, llength_folder)
            }

    for L in letters_nb:
        folders['{0}Letters'.format(L)] = os.path.join(folders['llength'],"{0}Letters".format(L))
...
def get_folder(sacc, rw_step, b_cell, added_noise, llength, letters_N):
    '''
    ...
    '''
    if sacc is None or rw_step is None:
        return folders['Results']
    else:
        folder = os.path.join(folders['Results'], 'Sacc={0}, RWS={1}'.format(sacc, rw_step))

        if not os.path.isdir(folder):
            raise ValueError('{0} is not a valid folder'.format(folder))

    if b_cell is None or added_noise is None:
        return folder
    else:
        folder = os.path.join(folder, 'Bcell={0}, added_noise={1}'.format(b_cell, added_noise))
...
def setup(letter_length_in_ms):
    global bipolar_cell_file, folders

    bipolar_cell_file = os.path.join(root_path, 'Inputs/bipolar_cell_{0}.txt'.format(bipolar_cell_nb))    # file exported from Igor

    folders = make_datafolders(letter_length_in_ms)
...
class cell:
    ...
    def _get_mp_noise_model(self, plot_flag=0):
        ...
        for i, contrast in enumerate(df['exp_contrast']):
            ...
            df.set_value(i, 'sim_mp_noise', df.get_value(i, 'sim_mp_sd')*df.get_value(i, 'exp_mp_noise')*added_noise_factor/df.get_value(i, 'exp_mp_sd'))
    ...
    def get_noise(self, shape, save_flag=1, corr_time=None):
        '''
        ...
        '''
        noise_file = os.path.join(folders['FEM'], 'linear_prediction_noise')
        #_ipdb.set_trace()
        try:
    ...
    def plot_noise(self, g, added_noise_factor, noise_corr_time, fig_g):
        ''' 
        ...
        noise = added_noise_factor * std * self.get_noise(g.shape, corr_time=noise_corr_time)
        tax = _get_simulation_TAX()
